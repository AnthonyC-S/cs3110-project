(** Represents type and functions of board used in the game, including 
    the initilizing of a new board, adding and removing of tiles from
    the board, and board validation and sorting. *)
...
(** [add_tile tile rl b] is a board [b'] that is board [b] with [tile] 
    ...
    and color that form a valid or run with the rest of the tiles in 
    its row. If no such values exists, joker tiles are left unaltered.
    Raises [RowAlreadyFull] if row [rl] is already full.  *)
val add_tile : Tile.t -> string -> b -> b

(** [remove_tile tile rl b] is a board [b'] that is board [b] with [tile] 
    removed from row [rl]. Joker tiles on the board are assigned a number 
